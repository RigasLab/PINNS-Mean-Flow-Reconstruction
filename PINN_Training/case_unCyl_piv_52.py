...
def add_noise(array, level):
    """Add noise to a field.
    """
    np.random.seed(10)
    noise_array = array + np.random.normal(0, level, size=array.shape)
    return noise_array


def main(train=True, test=True):
    # Case name
    case_name = "unCylinder_Foures_formulation_noise_level_0.02_0.02x0.02"
    case_name_title = r'PIV stride $0.02 \times 0.02$ Foures with 0.02 offset noise'
    ...
    # Add noise
    level = 0.02
    u_piv_noise = add_noise(u_piv, level)
    v_piv_noise = add_noise(v_piv, level)
    ...
    # BC objects
    u_piv_points = dde.PointSetBC(piv_points, u_piv_noise, component=0)
    v_piv_points = dde.PointSetBC(piv_points, v_piv_noise, component=1)
    ...
    if test:
        ...
        scipy.io.savemat(
            f"{case_name}/velocity_and_derivatives.mat", data_dict)
