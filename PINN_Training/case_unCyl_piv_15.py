...
def main(train=True, test=True):
    # Case name
    case_name = "unCylinder_Foures_formulation_history"
    ...
    if train:
        ...
        loss_update = dde.callbacks.LossUpdateCheckpoint(
            momentum=0.7,
            verbose=1, period=1, report_period=10,
            base_range=[0, 1, 2, 3],
        ...
        print(losshistory.loss_train)
        ar = np.array(losshistory.loss_train)
        np.savetxt(f"{case_name}/adam_train_history.txt", ar)
        ...
        print(losshistory.loss_train)
        ar = np.array(losshistory.loss_train)
        np.savetxt(f"{case_name}/bfgs_train_history.txt", ar)
    ...
    if test:
        ...
        # Finding (regressed) velocity derivatives
